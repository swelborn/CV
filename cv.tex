...
\usepackage[backend=biber, sorting=none, style=numeric, maxbibnames=30, giveninits=true, defernumbers=true]{biblatex}
...
\newcommand{\makeauthorsbold}[1]{%
  % Store the list of names to bold in a macro for \ifinlist to use.
  \def\namestobold{#1}%
  \DeclareNameFormat{author}{%
    % 1. Reset the boolean for each author.
    \setboolean{bold}{false}%
    % 2. Use \edef to make a 'hard copy' of the current author's family name.
    \edef\currentauthorlastname{\namepartfamily}%
    % 3. If the name starts with '*', remove it from our copy.
    \IfBeginWith{\currentauthorlastname}{*}{%
      \StrGobbleLeft{\currentauthorlastname}{1}[\currentauthorlastname]%
    }{}%
    % 4. Check if our cleaned name is in the list.
    % \expandafter is needed to make sure \ifinlist sees the *content* of
    % \currentauthorlastname, not the command itself.
    \expandafter\ifinlist\expandafter{\currentauthorlastname}{\namestobold}{%
      % If it's in the list, set the boolean to true.
      \setboolean{bold}{true}%
    }{%
      % If not in the list, do nothing.
    }%
    % 5. Format the output based on the final state of the 'bold' boolean.
...
\begin{itemize}
  ...
  \item Streaming from the Advanced Light Source (\href{https://als.lbl.gov/}{ALS}) to NERSC
    \begin{itemize}
      ...
      \item Enabled real-time tomography data reconstruction pipeline that reduces feedback time from \textgreater 10 minutes to \textbf{\textless 10 seconds}.
...
Summary: \textbf{8} first author, \textbf{12} co-author, \textbf{500+} citations (\href{https://scholar.google.com/citations?user=WsQfglgAAAAJ&hl=en&authuser=1}{Google Scholar})
